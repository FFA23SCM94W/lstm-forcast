...
# from keras import Input, Dense, LSTM, Dropout
...
#os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "/Users/jerrywee/work/iit/cs587/hw5/src/Flask_LSTM_React/LSTM-forecast/lstmforcast01-817d11f6de14.json"
...
def forecast():
    ...
    '''
    diff_time = max(X)[0] - firstDay

    Ys = [0, ]*(diff_time.astype('timedelta64[D]').astype(int) + 1)
    days = pd.Series([firstDay + np.timedelta64(i, 'D')
                      for i in range(len(Ys))])
    for x, y in zip(X, Y):
        idx = (x[0] - firstDay).astype('timedelta64[D]').astype(int)
        Ys[idx] = y
    ...
    history = model.fit(X_train, Y_train, epochs=20, batch_size=70, validation_data=(X_test, Y_test),
                        callbacks=[keras.callbacks.EarlyStopping(monitor='val_loss', patience=10)], verbose=1, shuffle=False)
    ...
    BASE_IMAGE_PATH = os.environ.get(
        'BASE_IMAGE_PATH', 'https://storage.googleapis.com/iit-587-hw5-lstm')
    # DO NOT DELETE "static/images" FOLDER as it is used to store figures/images generated by matplotlib
    #LOCAL_IMAGE_PATH = "/Users/jerrywee/work/iit/cs587/hw5/src/Flask_LSTM_React/LSTM-forecast/static/images/"
    LOCAL_IMAGE_PATH = "/app/static/images/"
    ...
    file_path = LOCAL_IMAGE_PATH + MODEL_LOSS_IMAGE_NAME
    createIfNotExist(file_path)
    ...
    # Save the figure in /static/images folder
    file_path = LOCAL_IMAGE_PATH + LSTM_GENERATED_IMAGE_NAME 
    createIfNotExist(file_path)
    plt.savefig(file_path)
    ...
    axs.set_ylabel('Issues')
    # Save the figure in /static/images folder
    file_path = LOCAL_IMAGE_PATH + ALL_ISSUES_DATA_IMAGE_NAME 
    createIfNotExist(file_path)
    plt.savefig(file_path)
